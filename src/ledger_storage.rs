...
impl LedgerStorage {
    ...
    pub async fn get_latest_stored_slot(&self, first_slot: Slot) -> Result<Slot> {
        // inc_new_counter_debug!("storage-hbase-query", 1);
        let mut hbase = self.connection.client();
        match hbase.get_last_row_key(&self.uploader_config.blocks_table_name).await {
            Ok(last_row_key) => {
                match key_to_slot(&last_row_key) {
                    Some(slot) => Ok(slot),
                    None => Err(Error::HBaseError(HBaseError::ObjectCorrupt(format!(
                        "Failed to parse row key '{}' as slot number",
                        last_row_key
                    )))),
                }
            },
            Err(HBaseError::RowNotFound) => {
                // If the table is empty, return a default value (e.g., first_slot - 1)
                Ok(first_slot - 1)
            },
            Err(e) => Err(Error::HBaseError(e)),
        }
    }
